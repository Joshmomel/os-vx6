...
void usertrap(void)
{
  ...
  {
    // printf("r_scause catch \n");
    uint64 fault_vaddr = r_stval();
    if (fault_vaddr >= MMAP_VASTART && fault_vaddr < MMAP_VEND)
    {
      ...
      uint64 vma_start = vma->start;

      if (vma->used == 0 || fault_vaddr >= vma->end)
      {
      ...
      uint64 file_start = vpage_base - vma_start;
